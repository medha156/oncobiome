...
## Implementation notes: Phases 1 & 2 as actually built in this repo

The Methodology section below documents the general best-practice approach
and literature basis for all 9 phases. This section documents the specific
choices made in this repo's actual Phase 1 and Phase 2 code, which go one
level more concrete than the general methodology.

### Phase 1 — choices made
...
*A note on data provenance in this repository:* the microbiome dataset was originally mislabeled `manor2020_microbiome` in `phase1_data_ingestion.py` — it is actually authored by Milani et al. [3], not "Manor." Similarly, the cancer-prevalence dataset was mislabeled `compton2020_cancer` — it is authored by Boddy et al. [2], with no "Compton" in the author list. Both were naming issues only (the underlying data and citations in this document were always correct); both have since been fixed in the code and data filenames (`milani2020_microbiome` / `milani2020_host_species.csv` and `boddy2020_cancer` / `boddy2020_cancer_prevalence.csv`), and Phase 1/2 were re-run to regenerate all outputs under the corrected names.
